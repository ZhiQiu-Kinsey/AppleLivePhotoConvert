...
## 2. 阶段路线（严格串行，每阶段一个 PR，分支逐级叠加）

| 阶段 | 分支 | 内容 | 状态 |
|---|---|---|---|
| 0 | `claude/exciting-knuth-n0b189` | Core 重构与数据安全（Media / Metadata / Pairing / Pipeline / Services） | 已完成，PR 待合并 |
| 1 | `…-phase1` | 桌面基础设施与外壳：DI、本地化单一数据源、弹窗服务、设置、任务中心、图库工作台（方案 A） | 进行中，见 [phase1-desktop-shell.md](phase1-desktop-shell.md) |
| 2 | `…-phase2` | 画廊：统一媒体扫描、等高排版引擎、缩略图分档与字节预算、DPI 感知、sRGB 与方向 | 待开始 |
| 3 | `…-phase3` | 播放：统一 iOS/安卓播放器（subfile、rawvideo BGRA、PTS、帧预算/环形缓冲）、HDR 色调映射、HDR 转码保真 | 待开始 |
| 4 | `…-phase4` | 瘦身对比（真实编码 + 1:1 放大镜）、依赖清单/校验/原子安装/能力探测 | 待开始 |
