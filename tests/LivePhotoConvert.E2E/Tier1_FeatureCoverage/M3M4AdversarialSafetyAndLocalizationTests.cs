...
/// </summary>
public class M3M4AdversarialSafetyAndLocalizationTests
{
    ...
    #region 3. SafetyGuard In-Place Backup, Rollback & Directory Resilience Tests

    [Fact]
    public void AtomicBackup_Lifecycle_CreatesAndDeletesBackup()
    {
        using var context = new E2ETestContext();
        var originalFile = context.CreateInputFile("ORIGINAL.jpg", [0x01, 0x02, 0x03, 0x04]);

        // 1. 创建备份
        var backupPath = SafetyGuard.CreateAtomicBackup(originalFile);
        Assert.Equal(originalFile + ".livephoto_backup", backupPath);
        Assert.True(File.Exists(originalFile));
        Assert.True(File.Exists(backupPath));
        Assert.Equal([0x01, 0x02, 0x03, 0x04], File.ReadAllBytes(backupPath));

        // 2. 清理备份
        SafetyGuard.DeleteAtomicBackup(backupPath);
        Assert.False(File.Exists(backupPath));
        Assert.True(File.Exists(originalFile));
    }

    [Fact]
    public void AtomicBackup_Restore_RollsBackContentWhenOriginalCorrupted()
    {
        using var context = new E2ETestContext();
        var originalFile = context.CreateInputFile("PHOTO_TO_OVERWRITE.jpg", [0xAA, 0xBB, 0xCC]);

        // 创建备份
        var backupPath = SafetyGuard.CreateAtomicBackup(originalFile);

        // 模拟就地覆盖写入了损坏的数据
        File.WriteAllBytes(originalFile, [0xFF, 0xEE]);
        Assert.Equal([0xFF, 0xEE], File.ReadAllBytes(originalFile));

        // 触发回滚恢复
        SafetyGuard.RestoreAtomicBackup(backupPath, originalFile);

        // 原文件成功还原为原始数据，且备份文件已被消费清理
        Assert.True(File.Exists(originalFile));
        Assert.Equal([0xAA, 0xBB, 0xCC], File.ReadAllBytes(originalFile));
        Assert.False(File.Exists(backupPath));
    }

    [Fact]
    public void AtomicBackup_Restore_RollsBackWhenOriginalDeleted()
    {
        using var context = new E2ETestContext();
        var originalFile = context.CreateInputFile("PHOTO_MISSING.jpg", [0x11, 0x22, 0x33]);
        ...
        Assert.True(File.Exists(originalFile));
        Assert.Equal([0x11, 0x22, 0x33], File.ReadAllBytes(originalFile));
        Assert.False(File.Exists(backupPath));
    }

    [Fact]
    public void AtomicBackup_OverwritesPreExistingStaleBackup()
    {
        using var context = new E2ETestContext();
        var originalFile = context.CreateInputFile("PHOTO_STALE.jpg", [0x99, 0x88]);
        var staleBackup = originalFile + ".livephoto_backup";
        File.WriteAllBytes(staleBackup, [0x00, 0x00, 0x00]); // 残留陈旧备份

        // 创建备份应自动替换陈旧备份
        var backupPath = SafetyGuard.CreateAtomicBackup(originalFile);

        Assert.Equal(staleBackup, backupPath);
        Assert.Equal([0x99, 0x88], File.ReadAllBytes(backupPath));
    }

    [Fact]
    public void AtomicBackup_RestoreWhenDirectoryMissing_HandlesGracefullyWithoutUncaughtCrash()
    {
        using var context = new E2ETestContext();
        var subDir = Path.Combine(context.InputDirectory, "sub_dir");
        Directory.CreateDirectory(subDir);
        var originalFile = Path.Combine(subDir, "TARGET.jpg");
        File.WriteAllBytes(originalFile, [0x55, 0x66]);
        ...
        // RestoreAtomicBackup 内部有 try-catch 保护，绝不抛出未经处理的未捕获崩溃
        var exception = Record.Exception(() => SafetyGuard.RestoreAtomicBackup(backupPath, originalFile));
    ...
    [Fact]
    public void AtomicBackup_RestoreWhenDirectoryMissing_FailsSilentlyWithoutCreatingDirectory_PreservesBackup()
    {
        using var context = new E2ETestContext();
        // 备份文件放在存在的根目录
        var backupPath = Path.Combine(context.InputDirectory, "source.jpg.livephoto_backup");
        File.WriteAllBytes(backupPath, [0x33, 0x44]);

        // 目标路径指向一个尚不存在的子目录
        var nonExistentSubDir = Path.Combine(context.InputDirectory, "new_or_deleted_subdir");
        var originalFileInMissingDir = Path.Combine(nonExistentSubDir, "TARGET2.jpg");
        Assert.False(Directory.Exists(nonExistentSubDir));

        // 尝试回滚：因为目标路径的父目录不存在，且 RestoreAtomicBackup 未调用 Directory.CreateDirectory
        SafetyGuard.RestoreAtomicBackup(backupPath, originalFileInMissingDir);

        // 发现：目标文件未被成功恢复（因为缺少父目录创建），且静默捕获了 DirectoryNotFoundException
        Assert.False(File.Exists(originalFileInMissingDir));

        // 防灾性良好的一面：因为 File.Copy 抛出异常，File.Delete(backupPath) 未被执行，备份依然安全保留
        Assert.True(File.Exists(backupPath));
        Assert.Equal([0x33, 0x44], File.ReadAllBytes(backupPath));
    }
