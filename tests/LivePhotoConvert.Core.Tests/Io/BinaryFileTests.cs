...
public class BinaryFileTests
{
    ...
    [Fact]
    public async Task Copying_To_An_Existing_File_Should_Not_Overwrite_It()
    {
        using var temp = new TempDirectory();
        var source = temp.CreateFile("data.bin", [0, 1, 2, 3]);
        var existing = temp.CreateFile("existing.bin", [9, 9]);

        await Assert.ThrowsAsync<IOException>(() => BinaryFile.CopySegmentAsync(source, existing, 0, 2, TestContext.Current.CancellationToken));
        Assert.Equal([9, 9], await File.ReadAllBytesAsync(existing, TestContext.Current.CancellationToken));
    }
}
