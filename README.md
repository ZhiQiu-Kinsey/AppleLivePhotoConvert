...
- 程序暂无代码签名，首次运行时 Windows SmartScreen 可能提示「未知发布者」，选择「仍要运行」即可。3.x 解压版用户需要手动安装一次 4.0.0，之后即可自动更新；原解压目录里已下载的依赖工具不会迁移到安装版，需要在依赖页重新下载。
...
- **源文件处置时机**：「移入备份文件夹」「移入回收站」「永久删除」只在输出落盘并校验通过后执行。永久删除需要输入 `DELETE` 确认。移入备份文件夹时，源文件移到其所在目录下的「已合成」或「已拆分」子文件夹（英文界面下为 `Merged` / `Split`，名称在任务开始时确定）。
